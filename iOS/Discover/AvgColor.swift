...
extension UIImage {
    ...
    func findAverageColor(_ algorithm: AverageColorAlgorithm = .simple) -> UIColor? {
        ...
        return UIColor(
            red: averageRed / 255,
            green: averageGreen / 255,
            blue: averageBlue / 255,
            alpha: 1
        )
    }
    
    private func red(for pixelData: UInt32) -> UInt8 {
        // For a quick primer on bit shifting and what we're doing here,
        // in our ARGB color format image each pixel's colors are stored as a 32 bit integer,
        // with 8 bits per color chanel (A, R, G, and B)
        //
        ...
        // So we're basically using it as a gate to only let the bits we want through
        // 255 (below) is the value we're using as in binary it's 11111111 (or in 32 bit, it's 00000000 00000000 00000000 11111111) and the result of the bitwise operation is then:
        ...
        // So as you can see, it only keeps the last 8 bits and 0s out the rest, which is what we want! Woohoo!
        // (It isn't too exciting in this scenario, but if it wasn't pure red and was instead a red of value "11010010" for instance,
        // it would also mirror that down)
