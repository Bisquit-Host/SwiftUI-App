...
struct AgentChatImageInput: Identifiable, Hashable, Encodable, Sendable {
    ...
    nonisolated init(id: UUID = UUID(), name: String, data: Data) throws {
        guard data.count <= Self.maxBytes else {
            throw AgentChatImageInputError.tooLarge(name)
        }

        guard let mediaType = Self.mediaType(for: data) else {
            throw AgentChatImageInputError.unsupported(name)
        }
    ...
    nonisolated static func from(url: URL) throws -> AgentChatImageInput {
        ...
        if let fileSize, fileSize > maxBytes {
            throw AgentChatImageInputError.tooLarge(url.lastPathComponent)
        }

        return try AgentChatImageInput(name: url.lastPathComponent, data: Data(contentsOf: url))
    }
